...
export default function Projects({ projects }: Props) {
  const orderedProjects = orderedProjectIds.map((projectId) =>
    projects.find((project) => project.id === projectId)
  );

  return (
    <div className="vwrap">
      <h3 className="absolute ml-4 justify-center top-24 text-center uppercase tracking-[20px] text-amber-500/40 text-2xl">
        Projects
      </h3>
              ...
              <div className="flex flex-col w-full space-y-4 justify-center items-center lg:pt-0">
                ...
                >
                  <a href={`${project?.link}`}>
                    ...
                    <div className="w-full h-32 md:h-[200px] lg:w-[500px] lg:h-[300px] object-cover justify-center items-center flex relative">
                      <Image
                        src={urlFor(project?.demoImage).url()}
                ...
                <a href={`${project?.link}`}>
                  <h4 className="text-2xl md:text-4xl font-semibold text-center">
                    ...
                    <span className="ul-gr">{project?.title}</span>
                  </h4>
                </a>
                {/* -↑- TITLE -↑- */}
                  ...
                  {project?.skills.map((skill, i) => (
                    <div key={skill._id} className="flex flex-col">
                      <Link href={`${project.link}`}>
                        <div className="w-[32px] h-[32px] md:w-[48px] md:h-[48px] relative border-[0.62px] border-[#fffa] rounded-full">
                          <Image
